...
public class TestHarness : MonoBehaviour
{
    ...
    [Range(0.0f, 1.0f)] public float BGMVolume = 0.5f;
    public GameObject GameplayRoom;
    public bool EnableGameplayRoom;
	...
	void Awake()
	{
	    ...
	    _camera.localPosition = new Vector3(0, 0.801f, -0.9f);
        _camera.localEulerAngles = new Vector3(34.7f, 0.313f, 0.08f);
	    _camera.localScale = Vector3.one;
	    Camera.main.nearClipPlane = 0.01f;
	    Camera.main.farClipPlane = 4.0f;
        ...
        fakeInfo.ActivateLights += delegate()
        {
            ...
            if (EnableBGM)
            {
                BGMSoundHandler(transform, false);
            }
            if (EnableGameplayRoom)
            {
                HandleGameplayRoom();
            }
            else
            {
                if (GameplayRoom != null && GameplayRoom.activeSelf == true)
                {
                    GameplayRoom.SetActive(false);
                }
            }
        };
        TurnLightsOff();
    ...
    void HandleGameplayRoom()
    {
        if (GameplayRoom != null)
        {
            if (GameplayRoom.activeSelf == false)
            {
                GameplayRoom.SetActive(true);
            }
            else
            GameplayRoom.transform.localPosition = new Vector3(0, -0.803f, 0.352f);
            GameplayRoom.transform.localEulerAngles = new Vector3(0, 0, 0);
        }
        else
        {
            Debug.LogError("Enable Gameplayroom was enabled, but a gameplay room hasn't been assigned.");
        }
    }
    ...
    private KMAudio.KMAudioRef BGMSoundHandler(Transform t, bool loop)
    {
        KMAudio.KMAudioRef audioRef = new KMAudio.KMAudioRef { StopSound = () => { } };

        AudioClip BGM = BackgroundMusic[Random.Range(0, BackgroundMusic.Count)];

        if (BGM != null)
        {
            audioSource.volume = BGMVolume / 5;
            audioSource.transform.position = t.position;
            audioSource.loop = loop;
        ...
        {
            Debug.LogError("Background music was enabled, but no track was selected; Not playing music.");
        }
        return audioRef;
    }
    ...
    private bool CheckingForMiddleClick;
    ...
    void Update()
    {
		...
		if (Input.GetMouseButtonDown(2))
		{
			// Get mouse origin
			mouseOrigin = Input.mousePosition;
            if (!CheckingForMiddleClick)
            {
                Debug.Log("Checking for middle click");
                CheckingForMiddleClick = true;
                StartCoroutine(CountMiddleClickTime());
            }
        }
        if (Input.GetMouseButtonUp(2))
        {
            if (CheckingForMiddleClick == true)
            {
                CheckingForMiddleClick = false;
                Debug.Log("Releasing before Pan Time, so resetting FOV...");
                Camera.main.fieldOfView = 60;
            }
            else Debug.Log("Releasing after Pan Time, so stopping the panning...");
        }
    ...
    IEnumerator CountMiddleClickTime()
    {
        for (int T = 0; T < 2; T++)
        {
            Debug.Log(T);
            if (T == 1)
            {
                ...
                else Debug.Log("CheckForMiddleClick was false.");
            }
    ...
    static bool PickedUpBomb = false;
    public static IEnumerator MoveCamera(Transform destination)
	{
        ...
        Vector3 bombPositionOrigin = Instance._bomb.localPosition;
		...
		Vector3 cameraDestination = new Vector3(0, 0.801f, -0.9f);

        Vector3 cameraOrientationOrigin = new Vector3(34.7f, 0.313f, 0.08f);
        //Vector3 cameraOrientationDestination = new Vector3(34.7f, 0.313f, 0.08f);
		...
		while ((Time.time - startTime) < moveTime)
		{
            ...
            if (!PickedUpBomb)
            {
                Instance._bomb.position = Vector3.Lerp(bombPositionOrigin, bombPositionOrigin + new Vector3(0, 0.229f, 0), (Time.time - startTime) / moveTime);
                Instance._bomb.localEulerAngles = Vector3.Lerp(bombOrigin, bombOrigin + new Vector3(-65, 0, 0), (Time.time - startTime) / moveTime);
                cameraDestination = new Vector3(0, 0.529f, -0.494f);
            }
            else
            {
                Instance._bomb.position = Vector3.Lerp(bombPositionOrigin, bombPositionOrigin - new Vector3(0, 0.229f, 0), (Time.time - startTime) / moveTime);
                Instance._bomb.localEulerAngles = Vector3.Lerp(bombOrigin, bombOrigin - new Vector3(-65, bombOrigin.y, bombOrigin.z), (Time.time - startTime) / moveTime);
                cameraDestination = new Vector3(0, 0.801f, -0.9f);
            }
            Instance._camera.localPosition = Vector3.Lerp(cameraOrigin, cameraDestination, (Time.time - startTime) / moveTime);
            //Instance._camera.localEulerAngles = Vector3.Lerp(cameraOrientationOrigin, cameraOrientationDestination, (Time.time - startTime) / moveTime);
            yield return null;
		}
        if (!PickedUpBomb)
        {
            PickedUpBomb = true;
            Instance._bomb.localEulerAngles = new Vector3(-65, 0, 0);
        }   
        else
        {
            PickedUpBomb = false;
            Instance._bomb.localEulerAngles = new Vector3(0, 0, 0);
        }
